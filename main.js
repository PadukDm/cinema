...
class Ticket {
    constructor(movie, cinemaHall, numSeats, ){
        this.movie = movie;
...
class CinemaHall{
    ...
    bookSeat(numSeats){
        if(this.availableSeats >= numSeats){
            ...
            console.log(`${numSeats}; seats booked successfully: Ticket number: ${ticket.ticketNumber}`)
            return ticket;
        }else{
            console.log(`Sorry! Not enough seats available`);
            return null;
        }
    }
...
bookingSystem.addMovie('Shrek', 90);
bookingSystem.addMovie('Alone at Home', 60);

console.log(bookingSystem.movies)
